...
class SmartHomeAPIRadioTool(CodedTool):
    """
    CodedTool implementation that calls an API to control the Radio.
    """
    def __init__(self):
        """
    ...
    def get_status_http(self) -> Union[str, Dict[str, Any]]:
        endpoint = self.base_http_url + "radio/status"
        try:
            response = requests.get(endpoint)
            response.raise_for_status()
            return response.json()  # Expected: {"radio": true/false}
        except Exception as e:
            return f"Error retrieving radio status via HTTP: {str(e)}"

    def set_state_http(self, desired_state: str) -> Union[str, Dict[str, Any]]:
        current_status = self.get_status_http()
        if isinstance(current_status, dict) and "radio" in current_status:
            current_state = "on" if current_status["radio"] else "off"
            if current_state == desired_state:
                return f"Radio is already {desired_state}."
        endpoint = self.base_http_url + "radio"
        ...
        try:
            ...
            return f"Error setting radio state via HTTP: {str(e)}"
