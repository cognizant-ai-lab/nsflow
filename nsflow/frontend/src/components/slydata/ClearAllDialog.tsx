  ...
  return (
    <Dialog open={open} onClose={onCancel} maxWidth="sm" fullWidth>
      ...
      <DialogContent sx={{ backgroundColor: theme.palette.background.paper, color: theme.palette.text.primary }}>
        <Typography color="warning.main" sx={{ mb: 2 }}>⚠️ This will permanently delete all SlyData including:</Typography>
        <Box sx={{ ml: 2, mb: 2 }}>
          <Typography>• {rootCount} root-level item{rootCount !== 1 ? 's' : ''}</Typography>
          <Typography>• All nested key-value pairs</Typography>
          <Typography>• Cached data in browser storage</Typography>
        </Box>
        <Typography>This action cannot be undone. Are you sure you want to clear all data?</Typography>
      </DialogContent>
