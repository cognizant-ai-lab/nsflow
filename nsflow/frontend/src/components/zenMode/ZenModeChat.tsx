...
const ZenModeChat = () => {
  ...
  // File attachments — shared hook + dialog state for the in-history viewer.
  const {
    attachedFiles,
    fileInputRef,
    openPicker,
    handleFileSelected: rawHandleFileSelected,
    removeAttachedFile,
    clearAttachments,
  ...
  const sendMessageWithText = async (messageText: string) => {
    ...
    // Inline text files + extract PDFs via /api/v1/process_pdfs (shared hook handles both).
    ...
    chatWs.send(JSON.stringify({ message: fullMessageForBackend }));
    setNewMessage("");
    clearAttachments();
  ...
  return (
    ...
    >
      ...
      >
        ...
        {attachedFiles.length > 0 && (
          <Box sx={{ mb: 1 }}>
            <AttachmentsPreview
              files={attachedFiles}
              onOpen={(f) => setViewingFile(f)}
              onRemove={removeAttachedFile}
            />
          </Box>
        )}

        {/* Hidden file input. The `cancel` event (attached via useEffect below) restores
            Zen Mode's fullscreen when the user dismisses the picker without selecting. */}
        <input
          ref={fileInputRef}
          type="file"
          accept=".pdf,.md,.txt"
      ...
      {/* File viewer dialog — opens when a user clicks an attached file chip. */}
